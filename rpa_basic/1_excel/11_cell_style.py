...
ws.column_dimensions["A"].width = 5 # A 열의 너비를 5로 설정
ws.row_dimensions[1].height = 50 # 1번째 줄의 높이를 50으로 설정

# 스타일 적용
a1.font = Font(color="FF0000", italic=True, bold=True) # 글자 색은 빨갛게, 이탤릭, 두껍게
b1.font = Font(color="CC33FF", name="Arial", strike=True) # Arial 폰트, 취소선 적용
...
# 70 점 넘는 셀에 대해서 초록색으로 적용
for row in ws.rows:
    for cell in row:
        cell.alignment = Alignment(horizontal="center", vertical="center")
        if cell.column == 1: # A 번호열은 제외
            continue
        # cell 이 저웃형 데이터이고 70점보다 높으면
